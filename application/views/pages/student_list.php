<!--<div class="col-md-3"></div>-->
<!--	<div class="col-md-9 body-container">-->
<!--		<div class="panel p-body">-->
<!--		<div class="panel-heading search">-->
<!--			<div class="col-md-6">						-->
<!--			-->
<!--			<h4 style="font-size:25px">Student List</h4>						-->
<!--			-->
<!--			-->
<!--			</div>-->
<!--		-->
<!--		</div>-->
<!--		<div class="panel-body">-->
<!--		<div class="table-responsive">-->
<!--				<table class="table table-striped table-bordered table-hover">-->
<!--					<tr>-->
<!--						<th>Student Id</th>-->
<!--						<th>Student Name</th>-->
<!--						<th>Course</th>-->
<!--						<th>Program</th>-->
<!--                        <th>Action</th>-->
<!--					</tr> -->
<!--					<tr>-->
<!--						<td>2011-00111</td>-->
<!--						<td>MILAGROS APACIBLE</td>-->
<!--						<td>BACHELOR OF SECONDARY EDUCATION</td>-->
<!--						<td>FULL MERIT</td>-->
<!--					<td><a class="a-table label label-info" href="index.php?page=editCHEDScholar">Edit<span class="glyphicon glyphicon-pencil"></span></a>-->
<!--						<a class="a-table label label-danger" href="index.php?page=deleteSchool">Delete <span class="glyphicon glyphicon-trash"></span></a>-->
<!--					</td>-->
<!--					</tr>-->
<!--					<tr>-->
<!--						<td>2010-00099</td>-->
<!--						<td>MAY LLANZA</td>-->
<!--						<td>3RD YEAR</td>-->
<!--						<td>FULL MERIT</td>-->
<!--					<td><a class="a-table label label-info" href="index.php?page=editCHEDScholar">Edit<span class="glyphicon glyphicon-pencil"></span></a>-->
<!--						<a class="a-table label label-danger" href="index.php?page=deleteSchool">Delete <span class="glyphicon glyphicon-trash"></span></a>-->
<!--					</td>-->
<!--					</tr>					-->
<!--					<tr>-->
<!--						<td>2013-00621</td>-->
<!--						<td>RAMIL TAMBIS</td>-->
<!--						<td>BACHELOR OF SCIENCE IN BUSINESS ADMINISTRATION</td>-->
<!--						<td>TULONG DUNONG</td>-->
<!--					<td><a class="a-table label label-info" href="index.php?page=editCHEDScholar"><span class="glyphicon glyphicon-pencil"></span></a>-->
<!--						<a class="a-table label label-danger" href="index.php?page=deleteSchool"><span class="glyphicon glyphicon-trash"></span></a>-->
<!--						<a class="a-table label label-danger" href="index.php?page=deleteSchool"><span class="glyphicon glyphicon-trash"></span></a>-->
<!--					</td>-->
<!--					</tr>					-->
<!--				</table>-->
<!--			</div>-->
<!--		</div>-->
<!--		</div>-->
<!--	</div>-->
<!--</div>-->
